...
const categorySelect=$("#categorySelect"),categoryOtherWrap=$("#categoryOtherWrap"),categoryOther=$("#categoryOther");
function syncCategoryOther(){if(!categorySelect||!categoryOtherWrap||!categoryOther)return;const custom=categorySelect.value==="__other";categoryOtherWrap.classList.toggle("hidden",!custom);categoryOther.required=custom;if(!custom)categoryOther.value=""}
if(categorySelect){categorySelect.addEventListener("change",syncCategoryOther);syncCategoryOther()}
...
document.querySelectorAll('[data-post-job]').forEach(b=>b.addEventListener("click",()=>setTimeout(()=>{loadCompany();syncCategoryOther()},0)));

function formData(){const f=$("#postForm"),d=Object.fromEntries(new FormData(f));delete d.companyLogo;if(d.category==="__other"){const custom=(d.categoryOther||"").trim();if(!custom)throw new Error("กรุณาระบุสายงานอื่น ๆ");d.category=custom}delete d.categoryOther;d.salaryMin=Number(d.salaryMin)||0;d.salaryMax=Number(d.salaryMax)||0;d.vacancies=Number(d.vacancies)||1;return d}
...
const postForm=$("#postForm");if(postForm)postForm.onsubmit=async e=>{e.preventDefault();const msg=$("#postMessage");if(msg)msg.textContent="";try{await createJob("pending");toast("ส่งประกาศแล้ว อยู่ระหว่างตรวจสอบ");postForm.reset();syncCategoryOther();await loadCompany();const d=$("#postDialog");if(d?.open)d.close();location.hash="employer"}catch(e){if(msg)msg.textContent=errText(e);toast(errText(e))}};
