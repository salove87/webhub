...
let manifest=[];
fetch('./jobs-static.json?ts='+Date.now()).then(r=>r.ok?r.json():[]).then(x=>{manifest=Array.isArray(x)?x:[]}).catch(()=>{});
...
document.addEventListener('click',e=>{const v=e.target.closest?.('[data-view]');if(!v)return;const id=v.dataset.view;const dialog=$('#jobDialog');if(dialog)dialog.dataset.jobId=id;setTimeout(()=>{const a=$('#applyBtn');if(a)a.dataset.jobId=id;enhance()},0)},true);
