...
@media(max-width:600px){#tips.tips-updates{padding:44px 16px 48px}#tips .tips-head{margin-bottom:18px}#tips .tips-head h2{font-size:28px}#tips .tips-head a{font-size:14px}#tips .tips-list{gap:22px!important}#tips .tips-side{grid-template-columns:1fr;gap:24px}#tips .tips-card h3{font-size:18px;line-height:1.5}#tips .tips-card small{font-size:13px}#tips .tips-featured h3{font-size:20px}}
`;document.head.appendChild(s)}
const media=(a,featured=false)=>{const icon=icons[a.category]||'📝';if(a.coverUrl)return `<div class="${featured?'tips-featured-media':'tips-card-media'}"><img src="${esc(a.coverUrl)}" alt="${esc(a.title||'บทความ')}" data-fallback-icon="${icon}" loading="lazy"></div>`;return `<div class="${featured?'tips-featured-media':'tips-card-media'}"><div class="${featured?'tips-featured-fallback':'tips-card-fallback'}">${icon}</div></div>`};
async function mount(){
 const sec=document.querySelector('#tips');if(!sec)return false;addCss();
 const list=sec.querySelector('.tips-list'),all=sec.querySelector('.tips-head a');
 if(all){all.href='articles.html';all.textContent='ดูทั้งหมด >'}if(!list)return true;
 list.innerHTML='<div class="tips-live-note">กำลังโหลดบทความล่าสุด…</div>';
 try{
  const snap=await getDocs(query(collection(db,'articles'),where('status','==','published')));
  const rows=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.publishedAt?.seconds||b.updatedAt?.seconds||0)-(a.publishedAt?.seconds||a.updatedAt?.seconds||0)).slice(0,7);
  if(!rows.length){list.innerHTML='<div class="tips-live-note">ยังไม่มีบทความที่เผยแพร่</div>';return true}
  const first=rows[0],rest=rows.slice(1);
  list.innerHTML=`<a class="tips-featured" href="${href(first)}">${media(first,true)}<small>${esc(first.category||'บทความ')}</small><h3>${esc(first.title||'บทความ')}</h3></a><div class="tips-side">${rest.map(a=>`<a class="tips-card" href="${href(a)}">${media(a)}<small>${esc(a.category||'บทความ')}</small><h3>${esc(a.title||'บทความ')}</h3></a>`).join('')}</div>`;
  list.querySelectorAll('.tips-featured-media img,.tips-card-media img').forEach(img=>{
   const fallback=()=>{if(!img.isConnected)return;const featured=!!img.closest('.tips-featured-media'),box=document.createElement('div');box.className=featured?'tips-featured-fallback':'tips-card-fallback';box.textContent=img.dataset.fallbackIcon||'📝';img.replaceWith(box)};
   img.addEventListener('error',fallback,{once:true});
   if(img.complete&&img.naturalWidth===0)fallback();
  });
 }catch(e){console.error('tips live articles',e);list.innerHTML='<div class="tips-live-note">ไม่สามารถโหลดบทความล่าสุดได้ในขณะนี้</div>'}
 return true
}
